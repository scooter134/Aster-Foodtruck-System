...
// PATCH /api/menu-items/fix-availability - Make all items available (utility endpoint)
router.patch('/fix-availability', async (req, res) => {
    try {
        const result = await pool.query(
            'UPDATE menu_items SET is_available = true WHERE is_available = false RETURNING menu_item_id'
        );
        res.json({ 
            success: true, 
            message: `Updated ${result.rowCount} items to available`,

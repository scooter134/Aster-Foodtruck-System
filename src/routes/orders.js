...
router.patch('/:id/status', async (req, res) => {
    try {
        ...
        // Try to update status history with additional details (optional, non-blocking)
        if (changed_by || change_reason) {
            try {
                await pool.query(`
                    ...
                    WHERE history_id = (
                        SELECT history_id FROM order_status_history 
                        WHERE order_id = $3 AND new_status = $4 
                        ORDER BY created_at DESC LIMIT 1
                    )
                `, [changed_by, change_reason, id, status]);
            } catch (historyError) {
                // Non-critical error, just log it
                console.log('Could not update status history:', historyError.message);
            }
        }
